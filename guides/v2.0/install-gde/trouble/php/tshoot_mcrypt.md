...
Particularly on developer systems that include a Linux/Apache/MySQL/PHP (LAMP) stack that is separate from the operating system, mcrypt may not be installed at all or is installed in the LAMP stack's path (not the operating system's path).

As a result, the Magento installer cannot locate the extension and the installation fails. This PHP misconfiguration issue can potentially be resolved by fixing the path or removing the line which loads mcrypt.
